#include "PingJNI.h" // Include the generated JNI header file#include <jni.h>
#include <stdio.h>
#include <sys/param.h>
...
#include <ctype.h>
#include <errno.h>
JNIEXPORT void JNICALL Java_PingJNI_ping(JNIEnv *env, jobject obj, jstring JNIString) {
    const char *CString = (*env)->GetStringUTFChars(env, JNIString, NULL); // Convert Java string to C string
       if (NULL == CString) return NULL;

    // Implement your ping logic using the targetStr and count parameters
    // For example, you can print the target and count values:
    printf("Pinging %s ...\n", CString);
/*
...
struct sockaddr whereto = CString;	/* who to ping */
...
/* counters */
static long npackets = 10;		/* max packets to transmit */
...
static unsigned long tsum;		/* sum of all times, for doing average */
...
main(int argc, char *argv[])
{
	...
	while ((ch = getopt(argc, argv, "I:LRc:dfh:w:l:nop:qrs:t:v")) != EOF)
		switch(ch) {
		case 'c':
			npackets = atoi(optarg);
			if (npackets <= 0) {
				(void)fprintf(stderr,
				    "ping: bad number of packets to transmit. Please enter more than zero.\n");
				exit(2);
			}
			if (npackets > 10) {
				(void)fprintf(stderr,
				    "ping: bad number of packets to transmit. Please enter less than 10.\n");
				exit(2);
			}
			break;
		case 'd':
	...
	if (argc != 1)
		usage();
	target = CString; //modified from target = *argv
	...
	packet = malloc((unsigned int)packlen);
	if (!packet) {
...
catcher(int ignore)
{
	...
	if (!npackets || ntransmitted < npackets)
		...
		(void)alarm((unsigned int)waittime);
	}
}
...
/*#endif*/
	...
	if (icp->icmp_type == ICMP_ECHOREPLY) {
		...
		if (options & F_FLOOD)
			...
			/* check the data */
#ifndef icmp_data
			cp = ((unsigned char*)(icp + 1) + 8);
#else
			cp = (u_char*)icp->icmp_data + 8;
#endif
			dp = &outpack[8 + sizeof(struct timeval)];

			if (options & F_PRINTPAYLOAD)
			{
				if (strlen(dp) == 0)
					printf("(No exist payload data)");
				else
					printf("(Payload data is %s)", dp);
			}
...
/*#if 0*/
	...
	cp = (unsigned char *)buf + sizeof(struct iphdr);
	...
	for (; hlen > (int)sizeof(struct iphdr); --hlen, ++cp)
		...
		case IPOPT_RR:
			...
			if (i <= 0)
				continue;
			if (i == old_rrlen
			    && cp == (unsigned char *)buf + sizeof(struct iphdr) + 2
...
usage(void)
{
    ...
    (*env)->ReleaseStringUTFChars(env, JNIString, CString); // Release the allocated C string
}